...
from domain.services.product_extractors import ProductDetailsExtractor, ProductInfoExtractor
from domain.services.scraping_service import ProductCollector, ProductDetailCollector, ScrapingServiceInterface
...
class ProductCollectorImpl(ProductCollector):
    ...
    async def collect_products(self, page: Any) -> List[ProductBasicInfo]:
        ...
        for element in product_elements:
            try:
                product_info = await self._product_info_extractor.extract_basic_info(element)
                if product_info:
